...
__all__ = ["extensions", "on_load", "on_client_join", "on_client_leave", "on_data"]
...
class Extensions:
    ...
    def load_extensions(self):
        ...
        try:
            for i in os.listdir(self.EXTENSION_DIR):
                ...
                version = ".".join(
                    str(i) for i in ext_module.__extension_data__["version"]
                )
                Print.print_suc(
    ...
    async def handle_data(self, data: Data):
        await gather_funcs(
            func(data) for func in self.registed_data_handler.get(data.type, ())
        )
...
async def handle_get_extension_ids_req(data: Data):
    data.sender.send(
        format_data(  # type: ignore
            data.sender,
            "client.get-server-extension-ids_resp",
            {"UUID": data.content["UUID"], "Extensions": extensions.extension_ids},
        )
    )
