...
class Cfg:
    ...
    @staticmethod
    def get_cfg(path: str, standard_type: dict):
    ...
    @staticmethod
    def check_auto(standard: type | dict | list, val: Any, fromkey: Any = "?") -> None:
        """检查配置文件
        ...
        if isinstance(standard, type):
            ...
            Cfg.check_list_2(standard, val)
        elif isinstance(standard, dict):
            Cfg.check_dict_2(standard, val)
        else:
            raise ValueError(
    ...
    @staticmethod
    def check_dict_2(pattern: dict, jsondict: dict):
        """
        ...
        for key, std_val in pattern.items():
            if key == r"%any":
                ...
                for js_key, js_val in jsondict.items():
                    Cfg.check_auto(std_val, js_val, js_key)
            elif isinstance(key, Cfg.Group):
                # any key in members?
                js_val = FIND_NONE
                for member_key in key.members:
                    val_get = jsondict.get(member_key, FIND_NONE)
                    if val_get != FIND_NONE:
                        Cfg.check_auto(std_val, val_get, member_key)
            elif isinstance(key, Cfg.UnneccessaryKey):
                val_get = jsondict.get(key.key, FIND_NONE)
                if val_get != FIND_NONE:
                    Cfg.check_auto(std_val, val_get, key.key)
            else:
                val_get = jsondict.get(key, FIND_NONE)
                if val_get == FIND_NONE:
                    raise Cfg.ConfigKeyError(f"不存在的JSON键: {key}")
                Cfg.check_auto(std_val, val_get, key)

    @staticmethod
    def check_list_2(pattern: list, value: Any, fromkey: Any = "?") -> None:
        """检查列表
        ...
        if isinstance(pattern[0], str) and pattern[0].startswith(r"%list"):
            ...
            for val in value:
                Cfg.check_auto(pattern[1], val, fromkey)
        else:
            for single_type in pattern:
                try:
                    Cfg.check_auto(single_type, value, fromkey)
                    return
                except Exception:
                    pass
            raise Cfg.ConfigValueError(
